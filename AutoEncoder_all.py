...
parser.add_argument('--model_path', default="scpn_cifar10_ld_50_ep_294_va_0.9191.pth", type=str, help='checkpoint')
...
parser.add_argument('--cls_weight', type=float, default=1, help='ce learning weight')
...
def main(): # numclass=0
    ...
    run_dir = "./runs/" + CURRENT_TIME
    writer = SummaryWriter(run_dir, flush_secs=10)
    model = SparseAutoencoder_all(in_channel=args.data_channel,num_classes=args.num_classes,feature_dim=args.feature_dim, latent_dim=args.latent_dim) #
    ...
    for epoch in tqdm(range(args.epochs)):
        train_mse_loss, train_cls_loss, train_correct, train_total = 0, 0, 0, 0
        ...
        train_cls_loss = train_cls_loss / len(train_loader)  # / 938 = 60032 / 64
        ...
        writer.add_scalar("train/mse_loss", train_mse_loss, global_step=epoch)
        writer.add_scalar("train/cls_loss", train_cls_loss, global_step=epoch)
    ...
    #     x = view_data[i].reshape(1,1,28,28).to(device)
